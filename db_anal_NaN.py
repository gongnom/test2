...
output_file_path = os.path.join(os.path.dirname(input_file_path), 'column_analysis_with_included_na.xlsx')
...
for column in data.columns:
    ...
    if column == '상담분야':
        # 대괄호 외부의 ','로만 분리
        split_entries = column_data.dropna().apply(split_outside_brackets)
        # 리스트 평탄화
        all_entries = [item.strip() for sublist in split_entries for item in sublist]
        all_entries.extend(column_data[column_data.isna()].index.map(lambda _: "무응답"))  # '무응답' 포함
        # 개수 계산
        counts = pd.Series(all_entries).value_counts()
    else:
        counts = column_data.fillna("무응답").value_counts()
